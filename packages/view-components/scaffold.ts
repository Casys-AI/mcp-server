 ...
 * @example
 * ```sh
 * deno run -A jsr:@casys/mcp-view-components@0.7.0/scaffold result-viewer ./my-view
 * ```
 */
...
export function parseScaffoldArguments(args: readonly string[]): {
  ...
  if (positional[0] !== "result-viewer" || positional.length !== 2) {
    throw new ScaffoldError(
      "Usage: mcp-view-components scaffold result-viewer <target> [--force]. Example: deno run -A jsr:@casys/mcp-view-components@0.7.0/scaffold result-viewer ./result-viewer",
    );
  }
